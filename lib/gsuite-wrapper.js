...
exports.createAccount = async function (req, res, next) {
    ...
    if (!data.userPK
        ...
        try {
            ...
            log.warn(util.inspect(GsuiteError.errors));
            response = { success: false, errors: GsuiteError.errors, message: GsuiteError.errors[0].message };
            statusCode = GsuiteError.code;
        }
    }
 ...
 *         type: string
 *       - name: q
 *         description: The query (default ?q=orgUnitPath=/gsuiteWrapperTest) //TBD 
 *         in: query
 *         required: false
 *         type: string
 *       - name: sort
...
exports.listAccounts = async function (req, res, next) {
    ...
    const payload = {
        ...
        query: req.query.q || 'orgUnitPath=/gsuiteWrapperTest',
...
exports.updateAlias = async function (req, res, next) {
    // TODO Change the personPK to userPK. WHY DID THEY CHANGED IT??
    log.debug(req.headers['test-title']);
    ...
    if (!data.aliasName
        ...
        // Rigirio del cazzo. Ma chiamalo userPrimaryEmail, no?? Dove prendono la nomenclatura? Nei test o dove? Va definito uno standard. ANARCHIA.
        const userID = await redis.get('primary:' + personPK);
        log.debug(userID);
